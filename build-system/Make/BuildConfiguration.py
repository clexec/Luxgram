...
def copy_profiles_from_directory(source_path, destination_path, team_id, bundle_id):
    ...
    # Map of file names to target names (for fallback when bundle_id doesn't match)
    file_name_to_target = {
        'Telegram.mobileprovision': 'Telegram',
        'Intents.mobileprovision': 'Intents',
        ...
        'WatchApp.mobileprovision': 'WatchApp',
        'WatchExtension.mobileprovision': 'WatchExtension',
        ...
        if os.path.isfile(file_path):
            ...
            if profile_name.startswith(team_id + '.' + bundle_id):
                ...
                if profile_base_name in profile_name_mapping:
                    ...
                    matched_profiles = True
                else:
                    print('Warning: skipping provisioning profile at {} with bundle_id {} (base_name {})'.format(file_path, profile_name, profile_base_name))
            elif file_name in file_name_to_target:
                # Fallback: copy by filename if bundle_id doesn't match
                shutil.copyfile(file_path, destination_path + '/' + file_name_to_target[file_name] + '.mobileprovision')
                matched_profiles = True
                print('Warning: copied provisioning profile {} by filename (bundle_id mismatch: expected {}, got {})'.format(file_name, team_id + '.' + bundle_id, profile_name))
